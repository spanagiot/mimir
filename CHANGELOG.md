...
* [ENHANCEMENT] Ruler: Add `-ruler.concurrent-rule-evaluation` to allow independent rules of a tenant to be run concurrently. The amount of concurrency is controlled by `-ruler.max-concurrent-rule-evaluations` which can be configured on a per-tenant basis. A `-ruler.max-concurrent-rule-evaluations` value of `0` can be used to disable it for a particular tenant. By default, the new behaviour is disabled. #8146
